...
export default function Navbar() {
  ...
  // Highlight "Organizations" for any org-related path
  const orgsActive = location.pathname === '/organizations'
    || location.pathname.startsWith('/org/')

  // Highlight "Projects" for dashboard and project detail paths
  const projectsActive = location.pathname === '/dashboard'
  ...
  return (
    <nav className="bg-white border-b border-slate-200 px-4 sm:px-6 h-14 flex items-center shrink-0 sticky top-0 z-40">
      ...
      <div className="hidden sm:flex items-center gap-0.5">
        ...
        <Link to="/dashboard" className={linkClass(projectsActive)}>
          Projects
        </Link>
      </div>
